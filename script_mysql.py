...
### Insert command
@client.on(events.NewMessage(pattern="(?i)/input"))
async def insert(event):
    try:
        ...
        print(e)
        await client.send_message(chat_id, "Something Wrong happened... Check your code!", parse_mode='html')
        return
...
### SHOW COMMAND
@client.on(events.NewMessage(pattern="(?i)/show"))
...
if __name__ == '__main__':
    try:
        ...
        #if database already created, no need to do the command below, but if the database haven't got created, should uncommand the command below
